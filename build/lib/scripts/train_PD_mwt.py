r"""
Learned Primal-Dual algorithm applied to step-B in MicroWave Tomography image reconstruction.
in step-B, the task is to reconstruct the permittivity map x from the total electric field Et, which is an inverse
linear problem Et = Ax.
Here the dataset containing (Et, x) images are generated on the fly using random phantoms
====================================================================================================
This code is based on the DeepInverse implementation of the Unfolded Primal-Dual algorithm from
...
noise_level_img = 0.0 #0.05

# Generate the MWT operator.
physics = MWTstepB(
    img_width=img_size,
    no_of_receivers=16,
...
max_iter = 10 if torch.cuda.is_available() else 10  # number of unfolded layers
...
train_batch_size = 5
test_batch_size = 1
n_iter_training = int(1e5) if torch.cuda.is_available() else 100000
